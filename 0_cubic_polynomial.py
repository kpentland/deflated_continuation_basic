...
# here we solve for the root of a parameterised cubic polynomial using deflated
# continuation
...
def F(u,        # solution to try
    ...
    for u_star in u_stars:
        M *= np.abs(u - u_star)**(-p) + sigma

    return M*(q - (u - 1)*(u - 2)*(u - 3))
    
    
# define lambda range
# -----------------------
lmbda = np.linspace(-0.5, 0.5, 501)
...
for i in range(len(lmbda) - 1):
    ...
    for u in S[i]:
        try:
            root1 = newton(F, u, args=(S[i+1], q), maxiter=30, full_output=True, tol=1e-12)
